...
function Brand({ label, homeLabel, inverse = false }: { label: string; homeLabel: string; inverse?: boolean }) {
  return (
    <a href="#" aria-label={homeLabel} className="inline-flex items-center gap-3 rounded-lg focus-visible:outline-2 focus-visible:outline-pgys-blue">
      <span className={`grid size-9 place-items-center rounded-xl text-sm font-black ${inverse ? "bg-white text-pgys-navy" : "bg-pgys-blue text-white"}`} aria-hidden="true">
        P
      </span>
      <span className={`text-xl font-black tracking-[-0.04em] ${inverse ? "text-white" : "text-pgys-ink"}`}>{label}</span>
    </a>
  );
}

export function SiteHeader({ brandLabel, homeLabel, navigationLabel, mobileMenuLabel, links, cta }: HeaderProps) {
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <Container className="flex min-h-18 items-center justify-between gap-6">
        ...
        <nav aria-label={navigationLabel} className="hidden items-center gap-7 lg:flex">
          {links.map((link) => (
            <a key={link.href} href={link.href} className="rounded-md text-sm font-semibold text-pgys-slate transition-colors hover:text-pgys-blue focus-visible:outline-2 focus-visible:outline-pgys-blue">
              {link.label}
            </a>
          ))}
        ...
        <details className="group relative lg:hidden">
          ...
          <nav aria-label={navigationLabel} className="absolute right-0 top-14 flex w-72 flex-col gap-1 rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl shadow-slate-900/10">
            {links.map((link) => (
              <a key={link.href} href={link.href} className="rounded-xl px-4 py-3 text-sm font-semibold text-pgys-slate hover:bg-pgys-mist hover:text-pgys-blue">
                {link.label}
              </a>
            ))}
            <ButtonLink href={cta.href} className="mt-2">{cta.label}</ButtonLink>
          </nav>
        </details>
      </Container>
    </header>
...
export function HeroSection({ eyebrow, title, description, primaryCta, secondaryCta, proofPoints, panelLabel, panelStatus, panelItems }: HeroProps) {
  return (
    <section className="soft-glow relative overflow-hidden border-b border-slate-100" aria-labelledby="hero-title">
      ...
      <Container className="relative grid items-center gap-14 py-20 sm:py-24 lg:grid-cols-[1.05fr_.95fr] lg:py-30">
        <div>
          <p className="mb-6 inline-flex rounded-full border border-blue-200 bg-white/80 px-4 py-2 text-sm font-bold text-pgys-blue shadow-sm">{eyebrow}</p>
          <h1 id="hero-title" className="max-w-3xl text-balance text-4xl font-black leading-[1.08] tracking-[-0.055em] text-pgys-ink sm:text-5xl lg:text-6xl xl:text-7xl">
            {title}
          </h1>
          <p className="mt-7 max-w-2xl text-lg leading-8 text-pgys-slate sm:text-xl">{description}</p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <ButtonLink href={primaryCta.href}>{primaryCta.label}<ArrowIcon /></ButtonLink>
          ...
          <ul className="mt-9 flex flex-wrap gap-x-6 gap-y-3" aria-label="Engagements PGYS">
            {proofPoints.map((point) => (
              <li key={point} className="flex items-center gap-2 text-sm font-semibold text-pgys-slate"><CheckIcon />{point}</li>
            ))}
          </ul>
        </div>
        <div className="relative mx-auto w-full max-w-xl lg:mx-0 lg:justify-self-end">
          ...
          <div className="relative overflow-hidden rounded-[1.75rem] border border-white/80 bg-white/92 p-3 shadow-[0_28px_80px_rgba(15,23,42,0.16)] backdrop-blur">
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-4 sm:px-5">
              ...
              <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700"><span className="size-2 rounded-full bg-pgys-success" />{panelStatus}</span>
            </div>
...
export function ServicesSection({ eyebrow, title, description, services }: ServicesProps) {
  return (
    <section id="services" aria-labelledby="services-title" className="scroll-mt-24 bg-white py-20 sm:py-28">
      <Container>
        <SectionHeading eyebrow={eyebrow} title={title} description={description} titleId="services-title" />
...
function ServiceCard({ service }: { service: Service }) {
  return (
    <article className="group rounded-[1.5rem] border border-slate-200 bg-white p-6 transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:p-8">
      ...
      <ul className="mt-6 grid gap-3 border-t border-slate-100 pt-6 sm:grid-cols-3" aria-label={`Points clés de ${service.name}`}>
        {service.features.map((feature) => <li key={feature} className="flex gap-2 text-sm font-semibold text-pgys-ink"><CheckIcon />{feature}</li>)}
      </ul>
    </article>
  );
}
...
export function CtaSection({ eyebrow, title, description, primaryCta, secondaryText }: CtaProps) {
  return (
    <section id="contact" aria-labelledby="contact-title" className="scroll-mt-24 bg-white py-20 sm:py-28">
      <Container>
        <div className="relative overflow-hidden rounded-[2rem] bg-pgys-blue px-6 py-12 text-center shadow-[0_24px_70px_rgba(37,99,235,0.2)] sm:px-12 sm:py-16 lg:px-20">
          <div className="absolute -right-24 -top-32 size-80 rounded-full border-[60px] border-white/10" aria-hidden="true" />
          <div className="relative mx-auto max-w-3xl">
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-blue-100">{eyebrow}</p>
            <h2 id="contact-title" className="mt-4 text-balance text-3xl font-bold tracking-[-0.04em] text-white sm:text-4xl lg:text-5xl">{title}</h2>
            <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-blue-100">{description}</p>
            <div className="mt-8 flex flex-col items-center justify-center gap-4 sm:flex-row">
              <ButtonLink href={primaryCta.href} variant="light">{primaryCta.label}<ArrowIcon /></ButtonLink>
...
export function SiteFooter({ brandLabel, description, navigationLabel, columns, contactTitle, email, location, legalNote, copyright }: FooterProps) {
  return (
      ...
      <Container className="py-14 sm:py-16">
        <div className="grid gap-12 border-b border-white/10 pb-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
            ...
            {columns.map((column) => (
              <div key={column.title}>
                ...
                <ul className="mt-5 space-y-3">
                  {column.links.map((link) => <li key={`${column.title}-${link.label}`}><a href={link.href} className="rounded text-sm text-slate-400 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-white">{link.label}</a></li>)}
                </ul>
              </div>
            ))}
          </nav>
          <div>
            ...
            <a href={`mailto:${email}`} className="mt-5 block rounded text-sm text-slate-400 hover:text-white focus-visible:outline-2 focus-visible:outline-white">{email}</a>
...
function ServiceIcon({ name, size = "large" }: { name: ServiceIconName; size?: "small" | "large" }) {
  const iconClass = size === "small" ? "size-9 rounded-xl" : "size-12 rounded-2xl";
  const paths: Record<ServiceIconName, ReactNode> = {
    cloud: <><path d="M7 18a4 4 0 0 1-.4-7.98A6 6 0 0 1 18 8a5 5 0 0 1 0 10H7Z" /><path d="M12 12v7m0-7-2.5 2.5M12 12l2.5 2.5" /></>,
    ...
    backup: <><path d="M20 11a8 8 0 1 0-2.34 5.66" /><path d="M20 5v6h-6" /><path d="M12 8v4l3 2" /></>,
  };

  return <span className={`grid place-items-center bg-pgys-blue-light text-pgys-blue ${iconClass}`} aria-hidden="true"><svg viewBox="0 0 24 24" className={size === "small" ? "size-5" : "size-6"} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg></span>;
}

function CheckIcon() {
  return <svg aria-hidden="true" viewBox="0 0 20 20" className="mt-0.5 size-5 shrink-0 text-pgys-success" fill="none" stroke="currentColor" strokeWidth="2"><path d="m5 10 3 3 7-7" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

function ArrowIcon() {
  return <svg aria-hidden="true" viewBox="0 0 20 20" className="ml-2 size-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 10h12m-4-4 4 4-4 4" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}
