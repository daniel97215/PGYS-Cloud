...
const ALLOWED_QUERY_KEYS = ["search", "status", "page", "pageSize"] as const;
...
export async function GET(request: NextRequest) {
  ...
  let apiResponse = accessToken
    ? await fetchWorkspaces(accessToken, query)
    : null;
  ...
  if (!apiResponse || apiResponse.status === 401) {
    ...
    apiResponse = await fetchWorkspaces(accessToken, query);
  }
...
function fetchWorkspaces(accessToken: string, query: URLSearchParams) {
  const suffix = query.size ? `?${query.toString()}` : "";
  return fetch(`${platformApiBaseUrl}/platform/workspaces${suffix}`, {
