...
export async function GET(request: NextRequest) {
  ...
  let apiResponse = accessToken
    ? await fetchSubscriptions(accessToken, query)
    : null;
  ...
  if (!apiResponse || apiResponse.status === 401) {
    ...
    apiResponse = await fetchSubscriptions(accessToken, query);
  }
...
function fetchSubscriptions(accessToken: string, query: URLSearchParams) {
  const suffix = query.size ? `?${query.toString()}` : "";
  return fetch(`${platformApiBaseUrl}/platform/subscriptions${suffix}`, {
